		...
		if len(remoteBranches) > 1 {
			...
			if e.options.DefaultRemote != "" {
				...
				if hasDefaultRemoteBranch {
					...
					// 默认远程不包含该分支，记录详细信息
					e.log.Error("项目 %s 检出失败: 默认远程 '%s' 不包含分支 '%s'", project.Name, e.options.DefaultRemote, e.branchName)
				}
			} else {
				// 没有配置默认远程，返回错误
				e.log.Error("项目 %s 检出失败: 分支 '%s' 匹配多个远程跟踪分支，请使用 --default-remote 指定默认远程", project.Name, e.branchName)
			}

			// 输出可用的远程分支列表，帮助用户选择
			e.log.Info("项目 %s 的可用远程分支:", project.Name)
			for _, remoteBranch := range remoteBranches {
				if remoteBranch != "" {
