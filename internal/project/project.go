...
func (p *Project) Sync(opts SyncOptions) error {
	...
	if err != nil {
		logger.Error("项目 [%s] 检查失败: %v", p.Name, err)
	...
	if !opts.NetworkOnly {
		...
		if err != nil {
			logger.Error("项目 [%s] 工作区检查失败: %v", p.Name, err)
			return fmt.Errorf("检查工作区是否干净失败: %w", err)
		}
		
		// 如果有本地修改且不强制同步，报错
		if !clean && !opts.Force {
			logger.Warn("项目 [%s] 工作区不干净，需要使用 --force-sync 覆盖", p.Name)
			return fmt.Errorf("工作区不干净，使用 --force-sync 覆盖本地修改")
		}
		
		// 检出指定版本
		if err := p.GitRepo.Checkout(p.Revision); err != nil {
			logger.Error("项目 [%s] 检出 %s 失败: %v", p.Name, p.Revision, err)
			return fmt.Errorf("检出修订版本失败: %w", err)
		}
...
func (p *Project) SyncNetworkHalf(quiet bool, currentBranch bool, forceSync bool, noCloneBundle bool, 
	...
	if !exists {
		if !quiet {
			logger.Info("克隆项目 %s 从 %s", p.Name, p.RemoteURL)
		}
		
		// 创建父目录
		parentDir := filepath.Dir(p.Path)
		if err := os.MkdirAll(parentDir, 0755); err != nil {
			logger.Error("为项目 %s 创建目录 %s 失败: %v", p.Name, parentDir, err)
			return false
		}
...
func (p *Project) SyncLocalHalf(detach bool, forceSync bool, forceOverwrite bool) bool {
	...
	// 如果当前分支与目标分支不同，或者强制检出
	if currentBranch != p.Revision || forceSync || forceOverwrite {
		logger.Debug("检出项目 %s 的修订版本 %s", p.Name, p.Revision)
		
		// 检出指定版本
		if err := p.GitRepo.Checkout(p.Revision); err != nil {
			logger.Error("检出项目 %s 的修订版本 %s 失败: %v", p.Name, p.Revision, err)
			return false
		}
	} else {
