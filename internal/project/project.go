...
func (p *Project) SyncNetworkHalf(quiet bool, currentBranch bool, forceSync bool, noCloneBundle bool, 
	...
	logger.Debug("开始执行项�?%s 的网络同�?, p.Name)
	
	// 检查项目目录是否存�?
	exists, err := p.GitRepo.Exists()
	if err != nil {
		logger.Error("检查项�?%s 是否存在失败: %v", p.Name, err)
		return false
	}
...
func (p *Project) SyncLocalHalf(detach bool, forceSync bool, forceOverwrite bool) bool {
	logger.Debug("开始执行项�?%s 的本地同�?, p.Name)
	
	// 检查是否有本地修改
	clean, err := p.GitRepo.IsClean()
	if err != nil {
		logger.Error("检查项�?%s 工作区是否干净失败: %v", p.Name, err)
		return false
	}
	
	// 如果有本地修改且不强制同步，报错
	if !clean && !forceSync && !forceOverwrite {
		logger.Warn("项目 %s 工作区不干净，需要使�?--force-sync 覆盖本地修改", p.Name)
		return false
	}
	...
	if currentBranch != p.Revision || forceSync || forceOverwrite {
		...
		logger.Debug("项目 %s 已经在正确的修订版本 %s �?, p.Name, p.Revision)
	}
...
func (p *Project) GetStatus() (string, error) {
	logger.Debug("获取项目 %s 的状�?, p.Name)
	
	status, err := p.GitRepo.Status()
	if err != nil {
		logger.Error("获取项目 %s 状态失�? %v", p.Name, err)
		return "", fmt.Errorf("获取项目状态失�? %w", err)
	}
...
func (p *Project) DeleteWorktree(quiet bool, forceRemoveDirty bool) error {
	...
	if !forceRemoveDirty {
		...
		if !clean {
			logger.Warn("项目 %s 工作区不干净，需要使�?--force-remove-dirty 强制删除", p.Name)
			return fmt.Errorf("工作区不干净，使�?--force-remove-dirty 强制删除")
		}
	}
	...
	if err := os.RemoveAll(p.Worktree); err != nil {
		...
		return fmt.Errorf("删除工作树失�? %w", err)
	}
	
	logger.Debug("项目 %s 的工作树已删�?, p.Name)
	return nil
}

// GetCurrentBranch 获取当前分支
func (p *Project) GetCurrentBranch() (string, error) {
	logger.Debug("获取项目 %s 的当前分�?, p.Name)
...
func (p *Project) HasBranch(branch string) (bool, error) {
	logger.Debug("检查项�?%s 是否有分�?%s", p.Name, branch)
	
	output, err := p.GitRepo.RunCommand("branch", "--list", branch)
	if err != nil {
		logger.Error("列出项目 %s 的分支失�? %v", p.Name, err)
...
func (p *Project) DeleteBranch(branch string) error {
	logger.Debug("准备删除项目 %s 的分�?%s", p.Name, branch)
	
	if branch == "" {
		logger.Error("尝试删除项目 %s 的空分支�?, p.Name)
		return fmt.Errorf("分支名为�?)
	}
	...
	if !exists {
		...
		return fmt.Errorf("分支 '%s' 不存�?, branch)
	}
	
	// 删除分支
	output, err := p.GitRepo.RunCommand("branch", "-D", branch)
	if err != nil {
		logger.Error("删除项目 %s 的分�?%s 失败: %v\n%s", p.Name, branch, err, output)
		return fmt.Errorf("删除分支失败: %w\n%s", err, output)
	}
	
	logger.Debug("已删除项�?%s 的分�?%s", p.Name, branch)
	return nil
}
